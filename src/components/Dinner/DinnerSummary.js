...
const DinnerSummary = () => {
    ...
    useEffect(() => {
        const fetchSummary = async () => {
            try {
                const res = await fetch(`${process.env.REACT_APP_BACKEND_URL}/admin/dinner-report`, {
                    headers: {
                ...
                if (!data.success) {
                    message.warning(data.message || "Dinner summary not available yet.");
                } else {
                ...
                message.error("Failed to load dinner summary.");
            } finally {
                setLoading(false);
            }
        };
        ...
        <div className="relative p-2 sm:p-4 md:p-8 flex justify-center items-start">
            <Card
                style={{
                    ...
                    padding: "16px",
                    background: "linear-gradient(145deg, #e6f4ff, #ffffff)", // polished light-blue tone
                }}
            >
                <h2
            ...
            <style jsx="true">{`
                .ant-table-thead > tr > th {
                    background-color: #003a8c !important; /* deep blue header */
                    color: #fff !important;
                    font-weight: 600;
